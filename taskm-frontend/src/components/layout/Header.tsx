...
import { 
    Bell, Settings, LogOut, Menu, X, User, Briefcase, 
    LayoutDashboard, CheckSquare, FolderKanban, Clock, UserCog 
} from 'lucide-react';
...
export const Header: React.FC = () => {
  ...
  useEffect(() => {
    const loadNotifs = async () => { if (!authState.user) return; try { const res = await axios.get('/api/notifications', { withCredentials: true }); setNotifs(Array.isArray(res.data) ? res.data : []); } catch (_) {} };
    loadNotifs();
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[60] lg:hidden" onClick={() => setShowMobileMenu(false)}>
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className="w-72 h-full bg-slate-900 border-r border-slate-700 p-6 flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-8">
                <Link to="/dashboard" className="flex items-center gap-3"><div className="p-2 bg-indigo-600 rounded-lg"><Briefcase className="w-6 h-6 text-white" /></div><h1 className="text-lg font-bold text-white">Workday</h1></Link>
